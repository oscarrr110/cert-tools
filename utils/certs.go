...
func newCertificate(org string, commonName string, years int) (*x509.Certificate, error) {
	now := time.Now()
...
// in the arguments.
func GenerateCACertificate(certFile, keyFile, org string, years int, bits int) error {
	template, err := newCertificate(org, "", years)
	if err != nil {
		return err
	}
...
func GenerateCert(hosts []string, certFile, keyFile, caFile, caKeyFile, org string,commonName string, years int, bits int) error {
	template, err := newCertificate(org, commonName, years)
	if err != nil {
		return err
	}
	// client
